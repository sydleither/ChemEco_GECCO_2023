...
namespace chemical_ecology {
  ...
  EMP_BUILD_CONFIG(Config,
    ...
    VALUE(DIFFUSION, double, 0.456, "Proportion of each population that diffuses to adjacent cells"),
    VALUE(SEEDING_PROB, double, 0.514, "Probability that a member of a given species will randomly be introduced to a cell"),
    VALUE(REPRO_THRESHOLD, double, 10000000, "Total population required for reproduction; expressed as a multiplier on MAX_POP"),
    VALUE(REPRO_DILUTION, double, .1, "Proportion of contents to propogate on reproduction"),
    VALUE(PROB_CLEAR, double, 0.003, "Probability of cell being cleared out"),    

    GROUP(INTERACTION_SETTINGS, "Interaction Sources"),
    VALUE(INTERACTION_SOURCE, std::string, "evolve/interaction_matrix4.dat", "Where to load interaction matrix from; empty string will generate randomly"),    
